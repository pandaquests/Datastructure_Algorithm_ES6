...
\begin{enumerate} 
	...
	\item Wir wollen die Zahlen aus dem entfernten Array-Teil nun am ursprünglichen Array anhängen. Wir nehmen die einzelnen Elemente des entfernten Array-Teils und hängen sie einzeln vorne am Ursprungsarray an. Wir können aber nicht das erste Element nehmen, sondern müssen mit dem letzten Element anfangen und uns zum ersten Element vorarbeiten. Daher drehen wir das Array mit Hilfe der \lstinline|reverse()| Funktion um und lassen dann eine \lstinline|forEach()| Funktion über das Array iterieren. 
...
\begin{itemize} 
	...
	Die Tatsache, dass man beim Array vom letzten Element anfangen muss ist in diesem Fall richtig. Jedoch ist es hier unnötig das Array mit der \lstinline|reverse()| Funktion umzukehren. Einfach und effizienter wäre es eine \lstinline|for| Schleife zu verwenden, die beim höchsten Index (also dem letzten Element) anfängt und sich dann vorarbeitet zum niedrigsten Index (also dem ersten Element).
...
let rotate_array = function(arr, n) {
	const rem = arr.splice(-1 * n);
	arr.reverse();
	rem.reverse().forEach(x => arr.push(x));
	arr.reverse();
};
\end{lstlisting}
